...
temp <- tables %>%
    ...
    mutate(metric=case_when(grepl('fpro$', metric)~'Ownership: For-profit',
        grepl('npro', metric)~'Ownership: Non-profit',
        grepl('govf', metric)~'Ownership: Gov., federal',
        grepl('govn', metric)~'Ownership: Gov., non-federal',
        TRUE~metric)) %>%
    mutate(metric=factor(metric, levels=c('Mergers', 'Closures', 'Openings', 'Hospitals', 'Beds', 'Teaching Hospitals', 'Ownership: For-profit', 'Ownership: Non-profit', 'Ownership: Gov., federal', 'Ownership: Gov., non-federal'), ordered=TRUE)) %>%
    arrange(metric)
```
