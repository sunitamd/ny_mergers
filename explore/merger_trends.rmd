...
aha <- aha %>%
    rename(merge_type=merge) %>%
    mutate(merge=case_when(!is.na(merge_type) ~ 1, TRUE ~ 0))

# Create NY state indicator
aha <- mutate(aha, ny=case_when(fstcd==36 ~ 1, TRUE ~ 0))
```
...
temp <- aha %>%
    ...
    summarise(merge_share=mean(sum(merge)/n())) %>%
    ungroup()
