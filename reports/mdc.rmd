...
mdc_spread <- spread(mdc, pay1, ds, fill=0) %>%
    mutate(total=Medicare+Medicaid+Private+SelfPay+NoCharge+Other,medicaid_pr=Medicaid/total, private_pr=Private/total)
mdc <- left_join(mdc, mdc_spread, by='mdc') %>%
    mutate(medicaid_pr=Medicaid/ds)
mdc_kmeans_data <- mdc_spread %>%
    mutate(Desc=mdc_names) %>%
    select(mdc, Desc, everything(), -medicaid_pr, -private_pr)
```
...
ggplot(mdc, aes(reorder(mdc, -medicaid_pr), ds, fill=pay1)) +
    geom_col() +
    scale_fill_manual('Payer', values=pay_cols) +
    labs(x='MDC', y='Discharges', caption='Arranged by descending Medicaid admissions proportion')
```
...
ggplot(mdc, aes(reorder(mdc, -Medicaid), ds, fill=pay1)) +
    geom_col() +
    scale_fill_manual('Payer', values=pay_cols) +
    labs(x='MDC', y='Discharges', caption='Arranged by descending Medicaid admission counts')
```
...
```{r, include=TRUE, cols.print=8, rows.print=5}
mdc_spread
```
...
```{r plotk2_k6, include=TRUE, fig.height=18, dpi=320}
grid.arrange(grobs=mdc_gplots, ncol=2)
```
